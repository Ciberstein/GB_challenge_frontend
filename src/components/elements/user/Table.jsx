...
const variants = {
  sm: { className: 'text-sm p-1 dark:border-zinc-800', size: 'small' },
  md: { className: 'text-md p-2 dark:border-zinc-800', size: '' },
  lg: { className: 'text-lg p-3 dark:border-zinc-800', size: 'large' },
  xl: { className: 'text-xl p-4 dark:border-zinc-800', size: 'large' },
};
        ...
        <tbody>
          { currentItems.length > 0
            ? currentItems.map((tr, index) => (
              <>
                <tr 
                  className={`border-b ${variants[size].className}
                    ${selectRow.active && 'hover:bg-gray-100 hover:dark:bg-slate-800 cursor-pointer'}
                    ${selectRow.selected === tr && 'bg-gray-100 dark:bg-slate-800 text-blue-400'}
                  `}
